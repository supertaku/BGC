...
export type SignageAnchor={id:string;building_id:string;position:Vec3;rotation:Vec3;width_m:number;height_m:number;sign_type:string;text:string;source_id:string;grounding:string;rights_status:string;visibility_priority:number};
...
export type M23Entry={id:string;kind:string;bounds:[number,number,number,number];url:string;activation_m?:number;entity_id?:string;name?:string;height_m?:number;replaces?:string[];focus?:Vec3;walk_position?:Vec3};
export type M23Catalog={schema_version:1;packages:M23Entry[];materials:M23Package['materials']};
export function m23Enabled(){return typeof window!=='undefined'&&new URLSearchParams(window.location.search).get('detail')==='m23';}
...
export async function applyM23Preview(world:WorldManifest,data:InteractiveManifest):Promise<[WorldManifest,InteractiveManifest]>{
 ...
 try {
 const [cat,variants]=await Promise.all([loadM23Catalog(),get('/world/detail/m23/tile-variants.json')]);
