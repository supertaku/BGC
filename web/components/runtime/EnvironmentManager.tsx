...
type LibraryEntry = { geometry: THREE.BufferGeometry; material: THREE.Material; elevation: number; scale: [number, number, number] };
...
function hashScale(id: string) { return .88 + (hash(id) % 25) / 100; }
...
function treeGeometry(variant: number) {
  ...
  const geometry = mergeGeometries([trunk, crown]);
  trunk.dispose();
  crown.dispose();
  return geometry;
}

function InstanceGroup({ items, entry, castShadow }: { items: EnvironmentInstance[]; entry: LibraryEntry; castShadow: boolean }) {
  const mesh = useRef<THREE.InstancedMesh>(null);
  const dummy = useMemo(() => new THREE.Object3D(), []);
  useLayoutEffect(() => {
    if (!mesh.current) return;
    items.forEach((item, index) => {
      const scale = hashScale(item.id);
      dummy.position.set(item.position[0], entry.elevation * scale, item.position[1]);
      dummy.rotation.set(0, (hash(item.id) % 360) * Math.PI / 180, 0);
...
export function EnvironmentManager({ activeIds, quality, refs, materials, onGroupCount }: {
  ...
  const library = useMemo<Record<string, LibraryEntry>>(() => ({
    TREE_A: { geometry: treeGeometry(0), material: materials.BGC_GRASS, elevation: 0, scale: [1, 1, 1] },
    TREE_B: { geometry: treeGeometry(1), material: materials.BGC_GRASS, elevation: 0, scale: [.95, 1.05, .95] },
    TREE_C: { geometry: treeGeometry(2), material: materials.BGC_GRASS, elevation: 0, scale: [1.05, .92, 1.05] },
  ...
  return <>{groups.map((group) => <InstanceGroup key={group.key} items={group.items} entry={group.entry} castShadow={quality === "FULL" && group.key.startsWith("TREE_")} />)}</>;
}
