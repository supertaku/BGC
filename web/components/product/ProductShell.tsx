...
type Panel = "search" | "place" | "help" | "about" | "graphics" | null;

export function ProductShell({ graphics,onGraphics,interactive, navigation, currentStreet, selectedEntity, tourStops, tourIndex, ready, onMode, onSelectPlace, onClosePlace, onTourStop, onShare, onFocusPlace }: {
  graphics:GraphicsPreset;onGraphics:(preset:GraphicsPreset)=>void;
  ...
  const graphicsButton=useRef<HTMLButtonElement>(null);
  ...
  function closePanel() {
    ...
    else if (panel === "graphics") graphicsButton.current?.focus();
    setPanel(null);
  }
  useEffect(() => {
  ...
  function openPanel(next: Panel) {
    ...
    if (navigation !== "INSPECT" && next !== "graphics") onMode("INSPECT");
  ...
  return <>
    <header className="product-header">
      ...
      <div className="header-actions"><button ref={graphicsButton} type="button" onClick={() => openPanel("graphics")} aria-expanded={panel === "graphics"}>Graphics</button><button ref={searchButton} type="button" onClick={() => openPanel("search")} aria-expanded={panel === "search"}>Search</button><button ref={helpButton} type="button" onClick={() => openPanel("help")} aria-expanded={panel === "help"}>Help</button><button ref={aboutButton} type="button" onClick={() => openPanel("about")} aria-expanded={panel === "about"}>About</button></div>
    </header>
    ...
    {navigation === "INSPECT" && !intro && !panel ? <div className="control-hint">Left drag — Pan <span>·</span> Right drag — Look around <span>·</span> Hold Right Mouse + WASD — Fly <span>·</span> Scroll — Zoom</div> : null}
    ...
    {panel === "help" ? <aside className="product-panel info-panel" aria-label="Help"><div className="panel-heading"><div><span className="eyebrow">Getting around</span><h2>Help</h2></div><button type="button" className="panel-close" onClick={closePanel}>Close</button></div><h3>Explore</h3><p>Left drag — Pan. Right drag — Look around. Hold Right Mouse + WASD — Fly. Scroll — Zoom. Select a building to see details. Search finds named places in the current dataset.</p><h3>Walk</h3><p>On desktop, select Walk and click the city to control the camera. Use WASD to move, Shift to move faster, and Esc to release the pointer.</p><h3>Tour</h3><p>Follow seven modeled landmarks using Previous and Next. Exit returns to Explore.</p></aside> : null}
    ...
    {panel === "graphics" ? <GraphicsPanel value={graphics} onChange={onGraphics} onClose={closePanel}/> : null}{!ready ? <div className="loading-pill" role="status">Preparing nearby buildings</div> : null}
  </>;
}
