...
export function ProductShell({ interactive, navigation, selectedEntity, tourStops, tourIndex, ready, onMode, onSelectPlace, onClosePlace, onTourStop, onShare, onFocusPlace }: {
  ...
  useEffect(() => {
    const keydown = (event: KeyboardEvent) => {
      ...
      if (panel) { event.preventDefault(); setPanel(null); if (panel === "search") searchButton.current?.focus(); else panelButton.current?.focus(); }
  ...
  function choose(entity: EntityRecord) { onSelectPlace(entity); setPanel("place"); setQuery(""); }
  ...
  return <>
    <header className="product-header">
      ...
      <div className="header-actions"><button ref={searchButton} type="button" onClick={() => openPanel("search")} aria-expanded={panel === "search"}>Search</button><button ref={panelButton} type="button" onClick={() => openPanel("help")} aria-expanded={panel === "help"}>Help</button><button type="button" onClick={() => openPanel("about")} aria-expanded={panel === "about"}>About</button></div>
    </header>
    ...
    {navigation === "WALK" ? <aside className="walk-help" aria-live="polite"><strong>{locked ? "Walking" : "Ready to walk"}</strong><span>{locked ? "WASD move · Shift faster · Esc release" : "Click the city to control the camera. WASD move, Shift faster, Esc release."}</span></aside> : null}
    ...
    {panel === "search" ? <aside className="product-panel search-panel" aria-label="Search named places"><div className="panel-heading"><div><span className="eyebrow">Find a place</span><h2>Search BGC</h2></div><button type="button" className="panel-close" aria-label="Close search" onClick={() => setPanel(null)}>Close</button></div><label htmlFor="place-search">Place name</label><input ref={inputRef} id="place-search" role="combobox" aria-autocomplete="list" aria-expanded={results.length > 0} aria-controls="place-results" aria-activedescendant={query && results[activeIndex] ? `place-option-${activeIndex}` : undefined} value={query} onChange={(event) => { setQuery(event.target.value); setActiveIndex(0); }} onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); setActiveIndex((index) => Math.min(results.length - 1, index + 1)); } else if (event.key === "ArrowUp") { event.preventDefault(); setActiveIndex((index) => Math.max(0, index - 1)); } else if (event.key === "Enter" && results[activeIndex]) { event.preventDefault(); choose(results[activeIndex]); } else if (event.key === "Escape") { event.preventDefault(); setPanel(null); searchButton.current?.focus(); } }} placeholder="Try Central Square" autoComplete="off" /><div id="place-results" role="listbox" className="search-results">{!query ? <p>Search {interactive.entities.length} named places in the current dataset.</p> : !results.length ? <p>No matching named place in the current BGC dataset.</p> : results.map((entity, index) => <button id={`place-option-${index}`} role="option" aria-selected={index === activeIndex} key={entity.entity_id} type="button" onMouseEnter={() => setActiveIndex(index)} onClick={() => choose(entity)}><strong>{entity.name}</strong><span>{typeLabel(entity.building_type)}</span></button>)}</div></aside> : null}

    {panel === "place" && selectedEntity && navigation !== "TOUR" ? <aside className="product-panel selection-panel" aria-label="Place details"><div className="panel-heading"><span className="eyebrow">Place details</span><button type="button" className="panel-close" onClick={() => { setPanel(null); onClosePlace(); }} aria-label="Close place details">Close</button></div><h2>{selectedEntity.name ?? "Unnamed building"}</h2><p className="place-type">{typeLabel(selectedEntity.building_type)}</p><dl><div><dt>Height</dt><dd>{heightLabel(selectedEntity)}</dd></div></dl><p className="data-note">This model uses mapped footprints. Background heights may be approximate.</p><div className="panel-actions"><button type="button" onClick={onFocusPlace}>View place</button><button type="button" onClick={onShare}>Share</button></div></aside> : null}

    {panel === "help" ? <aside className="product-panel info-panel" aria-label="Help"><div className="panel-heading"><div><span className="eyebrow">Getting around</span><h2>Help</h2></div><button type="button" className="panel-close" onClick={() => setPanel(null)}>Close</button></div><h3>Explore</h3><p>Drag to rotate, scroll to zoom, and select a building to see details. Search finds named places in the current dataset.</p><h3>Walk</h3><p>On desktop, select Walk and click the city to control the camera. Use WASD to move, Shift to move faster, and Esc to release the pointer.</p><h3>Tour</h3><p>Follow seven modeled landmarks using Previous and Next. Exit returns to Explore.</p></aside> : null}
    {panel === "about" ? <aside className="product-panel info-panel" aria-label="About and data"><div className="panel-heading"><div><span className="eyebrow">About the model</span><h2>About &amp; data</h2></div><button type="button" className="panel-close" onClick={() => setPanel(null)}>Close</button></div><p>BGC 3D is an exploratory city model built primarily from geographic open data. Some selected landmarks have additional modeling. Many background heights and generic street objects are approximate.</p><p>The project boundary is estimated. This experience is not survey grade and should not be used for legal or engineering decisions.</p><p>Current source snapshot: <code>{interactive.source_snapshot.split("/").pop()}</code>. Search covers {interactive.entities.length} named places.</p><button type="button" onClick={onShare}>Share BGC 3D</button></aside> : null}
