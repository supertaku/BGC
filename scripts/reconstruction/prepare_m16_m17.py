...
def cameras(feature: dict, asset_id: str, refs: list[str]) -> tuple[list[dict], list[dict]]:
    x0, y0, x1, y1 = shape(feature["geometry"]).bounds
    cx, cy = (x0 + x1) / 2, (y0 + y1) / 2
    span = max(x1 - x0, y1 - y0, 35)
    height = float(feature["properties"]["height_m"])
    specs = []
    visual = []
    for label, location, evidence in (
        ("north", [cx, y1 + span * 1.6, max(6, height * .42)], True),
        ("south", [cx, y0 - span * 1.6, max(6, height * .42)], False),
        ("east", [x1 + span * 1.6, cy, max(6, height * .42)], True),
        ("aerial", [cx + span * 1.2, cy - span * 1.2, max(24, height * 1.05)], True),
    ):
...
def package_target(target, volumes: list[dict], source: dict[str, dict]) -> dict:
    ...
    geometry = {"type": "FeatureCollection", "schema_version": 1, "entity_id": asset_id, "projected_crs": "EPSG:32651", "source_crs": "EPSG:4326", "local_origin_wgs84": [121.050972, 14.550806], "features": volumes}
    polygon_volumes = []
    edge_regions = []
    major_parts = []
    ...
    qa_specs, visual_cameras = cameras(primary, asset_id, ref_ids)
    observations = [
...
def main() -> None:
    ...
    dump(ROOT / "data/reports/m16-selected-buildings.json", {"schema_version": 1, "milestone": "M16", "generated_at": generated_at, "targets": selected, "evidence_gate": "PASS", "not_ready_replaced": 0})
    dump(ROOT / "data/batches/m17-batch.json", {"schema_version": 1, "milestone": "M17", "strategy": "Wave A regression gate before Wave B", "targets": selected})
    ...
    for item in selected:
        ...
        registry["buildings"][item["entity_id"]] = {"entity_id": item["entity_id"], "name": item["name"], "lifecycle": {"evidence": "RECONSTRUCTION_READY_WITH_GAPS", "lod2": "AVAILABLE", "lod1": "PENDING_VALIDATION"}, "source_feature_ids": source_ids, "runtime_merge_groups": {}, "available_lods": {"1": {"asset": f"exports/glb/buildings/{item['entity_id']}_lod1.glb", "manifest": f"data/assets/manifests/{item['entity_id']}_lod1.json", "metrics": f"data/reports/buildings/{item['entity_id']}.json", "version": "1.0", "status": "PENDING_VALIDATION"}, "2": {"representation": "WHOLE_BGC_TILE", "manifest": "exports/bgc/manifest.json", "source_feature_ids": source_ids, "generator": "blender/scripts/generate_bgc_tiles.py", "version": "1", "status": "AVAILABLE"}}}
    dump(registry_path, registry)
    tiers = defaultdict(int)
    for item in ranking:
        tiers[item["tier"]] += 1
    print(f"M16_PREP PASS candidates={len(features)} tier_a={tiers['A']} tier_b={tiers['B']} tier_c={tiers['C']} selected={len(selected)}")
