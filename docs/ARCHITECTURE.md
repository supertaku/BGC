...
## M23R/M24 completion pipeline

The existing base and M23 tile outputs remain prerequisites. Run `scripts/m23r/verify_reproducibility.py` with the project Python environment to compile M23 packages, add per-entity facade attributes to all 94 tile GLBs, compile evidence-backed signage/art metadata, and derive deterministic benchmark routes twice. It compares 317 generated files by SHA-256. `audit.py` checks vertical facade competition including sign planes; `completion_reports.py` writes the evidence summaries. Browser performance evidence is collected only after these outputs are stable.

At runtime `universalFacades.ts` shares one shader material across attributed building primitives. `M23Manager` keeps independently cached package geometries, uses owner-tile visibility for landmark identity, and commits replacements before hiding tile fallbacks. `LandmarkLODManager` applies the same ownership policy to the seven approved GLBs. `GroundSampler` remains the authority for Walk height. Sign textures are cached; ArtLayer uses neutral location markers when artwork reproduction is unsupported.

`ExploreControls` owns canvas pointer input. `GraphicsContext` supplies validated presets without remounting Canvas. `benchmarkMotion` is query-gated diagnostic movement with the same finite clock as `PerformanceProbe`; production navigation is unchanged when that diagnostic query is absent.
